...
EVENT_2026_FEEDBACK_URL=https://example.com/form
```

Первая включает кнопку внешнего фотоальбома. Вторая переводит страницу в режим переезда: основной временный контент скрывается и появляется ссылка на постоянную страницу события на церковном сайте. NFC URL при этом остаётся прежним. `EVENT_2026_FEEDBACK_URL` передаётся workflow как `NEXT_PUBLIC_EVENT_2026_FEEDBACK_URL` и показывает блок с кнопкой «Оставить отзыв». Без URL весь блок обратной связи не создаётся. Форма размещается во внешнем сервисе; сайт не собирает и не хранит ответы. Предполагаемые темы вопросов: что запомнилось, что можно улучшить и идеи для будущих выездов. Обязательный сбор имени, телефона, возраста или других персональных данных не предполагается.
...
Явный `posterPath` всегда имеет приоритет и использует тот же video origin. Автоматический browser-first-frame fallback по умолчанию не включён: для показа кадра каждый из шести `<video>` должен обратиться к MP4, а `preload="metadata"` является подсказкой браузеру и не гарантирует одинаковый объём загрузки в разных браузерах, при разном расположении MP4 metadata/keyframes и настройках Range Requests. Для production надёжнее один раз извлечь первый кадр, сохранить оптимизированный WebP/JPEG рядом с видео в S3 и указать его как `posterPath`. Если `posterPath` отсутствует, карточка сохраняет CSS-placeholder и не создаёт дополнительный preview-video.
